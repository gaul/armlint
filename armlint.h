...
// each exactly: 0 - x overflows only for x == INT_MIN and x + 1 only
// for x == INT_MAX, so the overflow flag of a compare against zero or
// against one is the equality test. The pair stays a flag-setting
// compare and a conditional branch, which fuse where cmp/b.cond fuse,
// and the constant is never materialized. The constant may sit in
// either CMP slot; the other operand is neither ZR nor the constant
// register, and the chain and CMP widths match. Only B.EQ/B.NE
// qualify; the ordered conditions need the subtraction. Every other
// constant is refused: the general eor Xc, Xn, #C ; cbz Xc form for
// any bitmask immediate is a size win that costs an issue slot on
// fusing cores, and stays a TODO.md candidate for an opt-in class.
// Emission is deferred on two fall-through proofs at once -- NZCV
// dead after the branch (the new compare sets N, Z and C from a
// different subtraction) and the constant register dead (the chain
// is deleted) -- run by armlint_advance_pending_bvs. Reported as
// "MOV + CMP + B.EQ/NE against INT_MIN/INT_MAX foldable to
// B.VS/B.VC". Runs before check_movz_movk_bitmask so the chain state
// is still active at the CMP.
...
// Advance the deferred B.VS/B.VC finding (check_mov_cmp_branch_bvs)
// by one instruction: the NZCV scan of armlint_advance_pending and
// the register scan of armlint_advance_pending_mz run side by side,
// and the finding emits once both have proven their value dead (an
// overwrite; for NZCV also a RET/BL/BLR). Either scan's read or
// unsafe terminator discards it. One instruction may settle both
// (ADDS into the constant register) or settle one while breaking the
// other (CSET into it reads the flags); the break wins. Sixteen
// undecided instructions expire it.
