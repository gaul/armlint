...
| ADD/SUB immediate chain leftovers | wider match | The adjacent fold is **done** (check_add_sub_imm_chain; see [analyses.md](analyses.md#addsub-immediate-chain-foldable-to-one) for the mechanism and the corpus figures). Of librustc_driver's 46,115 candidate chains armlint reports **26,825** (19,778 in the structural tier where the second ADD overwrites the intermediate, 26,337 in the deferred one). **The 19,290 difference is not a backlog**, and the 18,585 published here before it was both stale and miscategorized. It decomposes into two *correct and permanent* refusals: **5,551** candidates (12%) where a direct branch lands on the chain's first or second instruction, so folding would change behavior on the entry path; and roughly 13,700 more where the intermediate register is live, so the first ADD cannot be deleted and folding would give three instructions instead of two. (The two causes overlap, so they do not partition exactly.) Neither is reachable by a wider match -- both are the gate and the liveness scan working. ~~Still open, and unrelated to that number: non-adjacent chains~~ -- **done 2026-08-27, +771 findings** (26,893 -> 27,664, 770 of them librustc_driver), off **6,039** candidates corpus-wide (5,893 librustc_driver, 94 dyld, 29 libcrypto, 10 bash, 9 ssh, 4 go), measured 2026-08 with the intermediate untouched and no control transfer across the gap, gap 2 dominating at 3,277 of librustc's -- about one in eight survived the deadness proof and the side-entry gate, which is what that caveat predicted. Chains through a register copy are still unmeasured. One thing the suppressed set does point at, though it is a different candidate: where the intermediate is live, `add x9, x8, #a ; add x10, x9, #b` can still retarget the second to `add x10, x8, #(a+b)` -- two instructions either way, but the dependency chain breaks. That is the same size-neutral latency trade as the scratch-ADD pair row above, which failed the actionability bar for exactly that reason |
...
| Pattern | Rewrite | Notes |
| --- | --- | --- |
| `b.cond`/`cbz`/`tbz` `+8` over `b L` | `b.!cond L` | Implemented in 119c22e and reverted: sound (103/103 byte-verified), but clang/Mach-O emits the pair for conditional tail calls -- Mach-O has no conditional-branch relocation, so the spelling is forced and unfixable by recompiling. 2026-08 sweep: **507**, range-checked. The **2,437** published earlier was the shape count with no test of whether the rewrite can express its target, the same gap the scratch-ADD pair row had: the fold moves the destination from the unconditional branch's imm26 (+-128MB) onto the conditional's own field, imm19 (+-1MB) for `b.cond`/`cbz` and imm14 (**+-32KB**) for `tbz`. Split by form, `b.cond`/`cbz` reach 464 of 634 (73%) but `tbz`/`tbnz` reach **43 of 1,803 (2.4%)** -- and that is the idiom explaining itself, since `tbz +8 ; b L` exists precisely BECAUSE the target was outside TBZ's range, so folding it back is impossible almost by construction. 74% of the old population was never a candidate. Per binary: librustc_driver 2,270 -> 361, while dyld (9), go (59), bash (13) and ssh (2) reach in full -- which is why the /bin/bash census in issue #7 (12/12 tail calls) never met the constraint. shapescan reports both figures now (`(issue #7)` against `(target unchecked)`). Reinstatement options: opt-in/informational class, or suppress cross-symbol transfers via LC_FUNCTION_STARTS and keep only intra-function pairs (the function-starts/nlist parsing now exists in main.c -- symbolized findings use it -- so the suppression needs only the boundary check) |
...
| ~~ZR-operand ALU spellings~~ | ~~`mov` / `neg` / `mvn` / `mov #0`~~ | **Done, 2026-08-27: 90 findings**, exactly the swept population, all in librustc_driver: 67 `orr`, 14 `and`, 7 `add`, 2 `sub`. The alias table did not need enumerating after all -- every canonical degenerate spelling puts ZR in **Rn** (`mov` is `orr Rd, ZR, Rm`, `neg` is `sub Rd, ZR, Rm`, `mvn` is `orn Rd, ZR, Rm`), so keying on `Rm = 31` with `Rn != 31` excludes all of them at once. The S-variants are excluded as well; leaving the S bit free in a first version reported three `adds` in go and took the figure to 95, which is how the mask bug surfaced. `mul`/`bic`/`orn`/`eon` are decoded but occur nowhere in the corpus. See [analyses.md](analyses.md#zr-operand-alu-spelling-andorreorbicorneonaddsubmul-rd-rn-zr) |
...
| ~~`eor`/`sub vd, vn, vn`~~ | ~~`movi vd.2d, #0`~~ | **Done, 2026-08-27: 353 findings**, exactly the swept population -- this candidate has neither an encodability condition nor a deadness gate to lose sites to, the first row this month where the prediction landed on the nose. 351 are `eor Vd, Vd, Vd` and 2 in-place `orr`; no `sub` or `and` self-op occurs at all, so the row's title over-described it. 315 of the 353 sit in OpenSSL's `_asm_aescbc_sha1_hmac`, zeroing accumulators four at a time. The `orr` trap the row warned about is handled: `orr Vd, Vn, Vn` with `Rd != Rn` is the canonical vector MOV and only the in-place form is flagged. Guides checked before building: `MOVI #0` is on Neoverse V2's "Zero Latency MOVs" list (SWOG 4.12) and Apple Firestorm's rename-eliminated set, the self-op is on neither, and N1 documents no such elimination -- cheaper on newer cores, neutral on older. See [analyses.md](analyses.md#vector-self-op-identity-andorreorsubbic-vd-vn-vn) |
...
| Equality branch against a bitmask constant that no `cmp` immediate takes: `mov Xc, #C ; cmp Xn, Xc ; b.eq L` -> `eor Xc, Xn, #C ; cbz Xc, L` (`b.ne` -> `cbnz`) | **Shipped, then narrowed.** The [INT_MIN/INT_MAX fold](analyses.md#mov--cmp--beqne-against-int_minint_max-foldable-to-bvsbvc) first reported this general form for every bitmask constant (8115b14). The matching LLVM change ([llvm/llvm-project#223123](https://github.com/llvm/llvm-project/pull/223123)) drew the objection that `eor` + `cbz` is two issued instructions where `cmp`/`b.cond` fuse into one on most large cores (Cortex-A7x onward, Neoverse, Apple) and the `mov` can retire at rename, so the rewrite is a size win -- one instruction and the constant register -- that can cost an issue slot; LLVM kept only the sign-bit case, rewritten as `cmp xzr, Xn ; b.vs`, and armlint followed, adding `INT_MAX` via `cmn Xn, #1 ; b.vs`. What the general form adds under the same two proofs: **227** on Firefox 155 libxul.so (`0xffffff` 62, `0x3fffffff` 58, `0xffff` 34, `0xfffe000000000000` 12, `0x20000000` 10, a long tail), **463** on librustc_driver 1.97 (352 of them a `u16` against `0xffff`, `rustc_span`'s inline-`Span` interned marker, inlined into every `Span::data` caller), **11** on uutils -- against 1,424 / 179 / 5 for the extremes. The right home is an opt-in size class, the `-Os`/`-Oz` reading where LLVM could also take it under `optsize`; the narrowed check's value test is the one place to widen (the deleted predicate was "bitmask immediate but no add/sub immediate"), the two proofs and the rendering are unchanged. Do not promote to a default check |
...
were applied, and two are sharper warnings than the rest. The
dead-write row looked large, then small, then wrong, and only the
third pass showed most of what was left was a constant table being
read as code. The `movi` + vector-logic row went the other way: it was
implemented and tested before being reverted, because the conditions
that matter are the ones the *rewrite* imposes rather than the ones
the shape does -- and applying them took 40 to 12 to 0.
...
| `movi`/`mvni` + vector `and`/`orr`/`bic` | the immediate forms | **40 shape matches, 12 real candidates, 0 realized.** Implemented, measured, and reverted rather than shipped. The rewrite is BIC/ORR (vector, immediate), and three of its conditions the shape mask never applied. Those forms are **read-modify-write** (`Vd = Vd op #imm`), so the consumer must already be in place -- a separate source has nowhere to go, which rejects 10. BIC is **not commutative** (`Vn AND NOT Vm`), so its constant must sit in Vm. And the immediate encodes **one 8-bit byte lane** within a 16- or 32-bit element, which for AND means the *complement* of the mask must be that -- `and Vd, #0x000F` needs two byte positions cleared and so two instructions, saving nothing. That rejects 17 more. Of the 12 survivors, ten reuse the constant (loop-invariant masks in rustc) and two cannot be proven dead inside the liveness window, so the corpus yield is zero. The check itself is sound -- it emits `movi v1.8h, #0xff, lsl #8 ; and v0.16b, v0.16b, v1.16b` -> `bic v0.8h, #0xff` on a crafted site -- and the code is recoverable from this commit's parent if a corpus ever supports it. Note the earlier caution still holds: the MOVI mask must pin bits 28..19, since a looser one also matches `ushll`/`shll` and reported 1,662 |
...
## Driver and reporting (2026-09 x86lint cross-check)

armlint's driver was walked against
[x86lint](https://github.com/gaul/x86lint)'s, whose check table is
roughly half the size but whose ELF driver and reporting have grown
past this one's. Nothing here is a new analysis; it is all output,
scan scope and API, and every item already exists in working form on
the other side. Listed roughly in descending order of what it buys.

| Item | Notes |
| --- | --- |
| Restrict the scan to the symbol table's function ranges | The scan walks whole executable sections, so anything in them that *decodes* -- literal pools, jump tables, alignment islands -- is linted as code. This file already records what that costs: six of the ten surviving "pure write immediately clobbered" sites were four copies of SHA-1's round constant `0x5A827999` in libcrypto's and dyld's literal pools, and the LDR-literal row had to resolve and decode every target before it could say the premise was wrong. x86lint restricts to `.symtab` function ranges by default, tallies the excluded bytes into its skipped count, prints the restriction in the summary, and takes `-a` to scan everything anyway; it refuses `.dynsym` for this (exports only, so restricting to them would silently miss almost all code) and so stripped binaries scan whole sections exactly as now. armlint already parses both Mach-O `nlist`/`LC_FUNCTION_STARTS` and the ELF symbol tables for symbolization -- the boundaries are in hand, the scan simply does not gate on them. Caveat measured 2026-09: Debian's ELF arm64 binaries carry no `.symtab` at all, so the restriction is a no-op there, exactly as it is for a stripped x86 binary |
| BTI landing-pad verification (`-b`, the `-e` analog) | The largest genuinely new capability, and the one whose port is nearly mechanical. `GNU_PROPERTY_AARCH64_FEATURE_1_AND`'s BTI bit is the exact counterpart of x86's IBT bit, so a binary makes the same two claims that can drift apart: the note says every indirect target is padded, and the code either honours that or does not. All seven evidence classes carry over unchanged -- the entry point of a `PT_INTERP` program, `DT_INIT`/`DT_FINI`, the `{pre,}init_array` slots, `JUMP_SLOT`/`GLOB_DAT` relocations resolving in-object, `IRELATIVE` resolvers, every defined dynamic function symbol, and baked absolute pointers landing in an executable section (counted only when they address a known function entry, since a compare-only label legitimately has no pad) -- as do the four verdicts, including the one worth the most: pads present but the note lost, which the linker's AND produces from a single assembly object built without the flag. armlint's census already tallies BTI hints but never reconciles them against the note. AArch64 admits a sharper check than x86 has: `bti c`, `bti j` and `bti jc` say which transfer may land, so a `br` target wants `j` or `jc` and a `blr` target `c` or `jc`, and a pad of the wrong flavour is a finding x86 has no way to express |
| `--json` output and an `on_finding` callback | armlint has neither. The callback is the library half (name, absolute address, decoded form, raw bytes, one call per finding, independent of the summary and of `-v`) and `--json` is the driver half built on it. What it unlocks is joining findings against anything else keyed by address, and the join worth making is an execution profile: how often a compiler *emits* a suboptimal encoding is a poor proxy for what it costs, and one inside a JIT's dispatch loop outweighs thousands in cold init code. x86lint's `vaddr` is a number rather than a hex string for exactly that reason, and it streams findings as they are found rather than accumulating them |
| `-f NAME` single-function scan | "Lint just this function", drawing on `.dynsym` as well so an exported function of a stripped library can be named, and scanning every sized symbol spelled NAME (a versioned export has several). Composes with `-i` into the per-variant census: x86lint's worked example runs `-i -f` over each of glibc's three `memmove` IFUNC targets and reads off a different psABI level for each, which is the per-target question a whole-binary census cannot answer |
| By-function summary table | The top ten offending functions with the tail summed into a residue line, so a whole-binary count reads as "and here is where to start". armlint symbolizes individual `-v` findings already, so this is a tally over machinery that exists. Findings in code reached through an unsized assembly label want an honest `outside every function range` row rather than the nearest neighbour |
| Exit code 2 for tool failure | armlint returns 1 both when it finds opportunities and when the invocation or the file is bad, so CI cannot tell a dirty scan from a broken run. x86lint follows the grep convention: 0 clean, 1 findings, 2 tool failure |
| Skipped-word counter in the lint summary | The census reports undecodable words; the lint scan does not, so partial coverage of a data-laden region is indistinguishable from a clean sweep. x86lint exposes it as `x86lint_summary_skipped` on the library path as well |
| Census evidence labelling | The census asks the reader to treat small exotic tallies with suspicion and check `-v` samples by hand. x86lint automates exactly that: every tally is labelled against the bytes the toolchain recorded as code -- sized function symbols, `.eh_frame` FDE pc-ranges, Go `pclntab` boundaries -- a family with hits outside all of it prints as `FAM (n, u unevidenced)`, and a `code evidence` line reports what backed the labels. The semantics are deliberately asymmetric, inside evidence lending trust while unevidenced means only "no toolchain claim". `tools/cohere` is the record of why the cheap alternatives do not work: run length, gap adjacency and self-synchronization consensus all fail on ground truth, a GHC binary's phantom x87 reaching 92% consensus because self-sync is a decoder property and junk converges onto its own stable chain |
| Go `pclntab` parsing | A Go binary carries neither `nlist` nor `LC_FUNCTION_STARTS` nor a useful symbol table, so findings print unannotated and the census's pac-ret coverage line is omitted. The runtime's own function table survives `strip` because traceback needs it, and x86lint parses it across all four header eras back to Go 1.2, finding it by section name or by scanning the data sections and believing it only after the whole functab validates against the executable sections and the entry point (modern linkers store a dead `textStart` word, so the base is recovered from the section layout, and an externally linked binary whose candidates are all wrong drops the table honestly). Porting it gives armlint function boundaries, symbolization and pac-ret coverage on Go binaries at once |
