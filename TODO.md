...
| Multi-use base: fold every access off one ADD, then delete it | `check_add_ldr_imm_offset` now decodes both the scaled and the unscaled spelling of its consumer, so the whole candidate population is visible to it -- but only **126** of the 9,124 encodable ADD + LDUR/STUR sites became findings. The gap is not blindness, it is the deadness tier: the immediate tier is a load into its own base, which is 21.3% of the encodable *scaled* population and 1.4% of the unscaled one. The unscaled spelling appears where the offset is negative or off the access-size grid, which is a field access off a base the ADD does not consume -- at 10.2% of unscaled sites the very next instruction is another access off the same base, against 4.4% of scaled ones. The fold those want is N-for-1: if *every* use of the ADD's Rd is an access whose combined offset encodes, all of them can be rebased and the ADD deleted. That needs a multi-use forward scan rather than the adjacency-plus-single-consumer shape every current fold has. ~8,000 sites in the corpus, all of them already decoded and refused on soundness |
